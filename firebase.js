...
export async function ensureAuth() {
  ...
  try { return await _signing; } finally { _signing = null; }
}
...
export async function uploadImage(cardId, blob, fileName) {
  ...
  if (!/^image\//i.test(contentType)) throw new Error(`uploadImage: invalid content type: ${contentType}`);
  const path = `cards/${safeCardId}/${safeFileName}`;
  const storageRef = ref(_storage, path);
  await uploadBytes(storageRef, blob, { contentType, cacheControl: "public,max-age=31536000,immutable" });
...
export function getFirebaseInfo() {
  ...
  return {
    ...
    files: ["avatar.jpg", "logo.jpg", "banner.jpg", "photo1.jpg", "photo2.jpg", "photo3.jpg", "photo4.jpg", "photo5.jpg"]
  };
}
