/* ================================
   admin.js v491 (COMPLETE OVERWRITE)
   - Fetch card row by id from GAS
   - Build clean deliver links:
     1) Card clean: index.html?id=TW0001&view=1  (view=1 will be used by share/admin; your app.js can ignore it safely)
     2) Share card: share.html?id=TW0001  (if you have share gate page)
   - One-click delivery: ONLY output what you should hand to client
   - NEW: WeChat poster 1080x1920 canvas export (+ iOS open-image fallback)
================================== */
...
function baseDir_(){
  // same folder as admin.html, but we want index.html path in same folder
...
function shareUrl_(id){
  const cid = normalizeId_(id) || CONFIG.DEFAULT_ID;
  // if you don't have share.html, keep it anyway; you can add later
...
/* ================================
   WeChat Poster 1080x1920
================================== */
function pickPosterPhotos_(row){
  const urls = [];

  // prefer photos_fast bulk
  ...
  for(const u of urls){
    ...
    seen.add(key);
    out.push(nu);
    if(out.length >= 3) break; // poster use up to 3 photos
  }
  return out;
}
...
async function generateWeChatPoster_(){
  ...
  if(slogan){
    ...
    ctx.fillText(l1, bx + 22, by + 62);
    if(l2) ctx.fillText(l2, bx + 22, by + 112);
  ...
  // enable open poster (for iOS long-press)
  const btnOpen = qs("btnOpenPoster");
  ...
  if(downloaded){
    alert("✅ 已生成微信長圖（若未自動存檔，請點「開啟長圖」再長按存圖）");
  }else{
    alert("✅ 已生成微信長圖（請點「開啟長圖」→ 長按圖片存圖）");
  }
}
