...
const slot = `photo${i}`;
const hidden = document.getElementById(`${slot}_url`) || document.getElementById(slot.replace("photo", "photo") + "_url");
const input = document.getElementById(`file_${slot}`);
if (hidden) hidden.value = "";
const thumb = document.getElementById(`thumb_${slot}`);
if (thumb) thumb.textContent = "尚未選擇圖片";
const status = document.getElementById(`slotStatus_${slot}`);
if (status) status.textContent = "未開放";
...
if (pageIndex === 1 && !getFieldValue("name")) throw new Error("請填寫姓名或品牌名稱。");
if (pageIndex === 2) {
  const contactOk = [getFieldValue("phone"),getFieldValue("email"),getFieldValue("line_url"),getFieldValue("line_oa"),getFieldValue("wechat_id")].some(Boolean);
...
if (pageIndex === 3) {
  for (let i = 1; i <= state.rules.maxCtas; i++) {
    ...
    if ((t && !l) || (!t && l)) throw new Error(`第 ${i} 個按鈕請同時填寫文字和連結。`);
  }
}
setStatus("請繼續填寫。");
return true;
...
const map = {'s1':'style-arch','s2':'style-flat','s3':'style-spot','arch':'style-arch','flat':'style-flat','spot':'style-spot'};
...
const allThemeClasses = ['mode-free','mode-premium','color-1','color-2','color-3','color-4','color-5','style-arch','style-flat','style-spot','paper-1','paper-2','paper-3','p1','p2','p3','p4','p5','p6','p7'];
el.previewCard.classList.remove(...allThemeClasses);
const currentPlan = state.plan;
if (currentPlan === 'premium') {
  el.previewCard.classList.add('mode-premium');
  el.previewCard.classList.add(mapPremiumToTheme(getFieldValue("premium_color")));
} else {
  el.previewCard.classList.add('mode-free');
  el.previewCard.classList.add(mapFreeColorToTheme(getFieldValue("color")));
  el.previewCard.classList.add(mapStyleToTheme(getFieldValue("style")));
  el.previewCard.classList.add(mapPaperToTheme(getFieldValue("paper")));
}
}
...
if (state.cropper.imageUrl) { try { URL.revokeObjectURL(state.cropper.imageUrl); } catch {} }
...
el.cropMeta.textContent = cfg.cropShape === "square" ? "頭像與 Logo 建議主體置中。" : "照片可先縮小保留更多畫面，也可放大讓主體更明顯。";
...
try {
  ...
  const formData = { name: getFieldValue("name") };
  const plan = normalizePlan(getFieldValue("plan"));
  const planName = plan === "premium" ? "精品設計款" : "自由搭配款";
  const planAmount = Number(res.plan_amount ?? res.data?.plan_amount ?? (plan === "premium" ? 2000 : 1500)) || 0;
  const addonAmount = Number(res.addon_amount ?? res.data?.addon_amount ?? 0) || 0;
  const totalAmount = Number(res.total_amount ?? res.data?.total_amount ?? (planAmount + addonAmount)) || (planAmount + addonAmount);
...
function hideSuccess() { toggleEl(el.successBox, false); }

function resetForm() {
if (state.busy) return;
if (!window.confirm("確定要重設這份表單嗎？")) return;
const allInputIds = ["plan","color","style","paper","premium_color","name","unit","title","slogan","services","experience","phone","email","line_url","line_oa","wechat_id","website","address","video1","video2","video3","social1","social2","social3","cta_text_1","cta_link_1","cta_text_2","cta_link_2","cta_text_3","cta_link_3"];
...
state.busy = !!busy;
[el.prevBtn, el.nextBtn, el.btnSubmit, el.btnReset, el.btnTest, el.cropApply, el.cropCancel, el.cropCenter, el.cropReset, el.cropZoomIn, el.cropZoomOut].forEach((node) => { if (node) node.disabled = !!busy; });
...
function openModal(modal) { if (!modal) return; modal.classList.add("show"); document.body.classList.add("modal-open"); }
