...
(() => {
  ...
  const CONFIG = {
    ...
    DRAFT_KEY: "hsc_form_draft_v682",
    PLAN_PRICES: { free: 1500, premium: 2000 },
    BASE_LIMITS: { free: { photos: 2, ctas: 1 }, premium: { photos: 5, ctas: 3 } }
  };
  const state = { plan:null, addons:new Set(), photoQty:0, ctaQty:0, photoLimit:0, ctaLimit:0, planAmount:0, addonAmount:0, totalAmount:0, submitting:false };
  const els = {};
  ...
  function init() {
    ...
    restoreDraft();
    syncAll();
    setStatus("目前版本：v6.8.2 真・商業閉環完整版");
  }
  function cache() {
    const ids = ["smart-card-form","form-status-strip","btn-open-showcase","summary-plan-pill","summary-photo-pill","summary-cta-pill","summary-plan-name","summary-photo-count","summary-cta-count","summary-marquee-status","quote-state-text","quote-plan-amount","quote-addon-amount","quote-total-amount","quote-addon-breakdown","free-theme-group","premium-theme-group","addon_photo_enabled","addon_photo_qty","addon_cta_enabled","addon_cta_qty","addon-photo-tip","photo-slots","cta-slots","photo-slot-template","cta-slot-template","marquee-section","btn-save-draft","btn-clear-draft","btn-contact-service","btn-submit-form","btn-gold-info","btn-gold-copy","btn-gold-contact","display_name","title","phone","email","address","intro","free_color","free_style","free_paper","premium_color","marquee_1","marquee_2","marquee_3","invite_code","ref"];
  ...
  function bind() {
    ...
    els.smart_card_form.addEventListener("input", debounce(() => saveDraft(true), 300));
  }
  function getPlan() {
  ...
  function syncTheme() {
    ...
    document.querySelectorAll("[data-plan-card]").forEach(n => n.classList.toggle("is-selected", n.querySelector("input")?.checked));
  }
  function syncQty() {
  ...
  function calcTotals() {
    ...
    if (state.addons.has("addon_gold_member")) addon += 10000;
  ...
  function renderQuote() {
    ...
    if (state.addons.has("addon_gold_member")) rows.push(["金牌級會員", 10000]);
    els.quote_addon_breakdown.innerHTML = rows.length ? rows.map(([a,b]) => `<div class="quote-breakdown-row"><span>${a}</span><strong>${money(b)}</strong></div>`).join("") : "<p class='quote-breakdown-empty'>尚未選擇加購</p>";
  }
  function renderPhotos() {
    ...
    for (let i=1;i<=state.photoLimit;i++) {
      ...
      const img = frag.querySelector(".photo-preview-image");
      const empty = frag.querySelector(".photo-preview-empty");
      ...
      file.addEventListener("change", () => {
        ...
        rotation = 0; scale = 1; zoom.value = "1"; apply();
      });
      zoom.addEventListener("input", () => { scale = parseFloat(zoom.value) || 1; apply(); });
      left.addEventListener("click", () => { rotation -= 90; apply(); });
      right.addEventListener("click", () => { rotation += 90; apply(); });
      reset.addEventListener("click", () => { rotation = 0; scale = 1; zoom.value = "1"; apply(); });
      els.photo_slots.appendChild(frag);
    }
  }
  function renderCtas() {
    const prev = [...document.querySelectorAll(".cta-slot-card")].map(card => ({ label: card.querySelector(".cta-label-input")?.value || "", url: card.querySelector(".cta-url-input")?.value || "" }));
  ...
  function buildAddonItems() {
    ...
    if (state.addons.has("addon_bundle")) items.push({ name:"跑馬燈＋更新組合", amount:500 });
      ...
      if (state.addons.has("addon_marquee")) items.push({ name:"跑馬燈功能", amount:300 });
      if (state.addons.has("addon_update_unlimited")) items.push({ name:"無限更新", amount:300 });
    }
    if (state.photoQty) items.push({ name:"照片加購", qty:state.photoQty, unit_price:100, amount:state.photoQty*100 });
    if (state.ctaQty) items.push({ name:"CTA 加購", qty:state.ctaQty, unit_price:100, amount:state.ctaQty*100 });
    if (state.addons.has("addon_gold_member")) items.push({ name:"金牌級會員", amount:10000 });
    return items;
  }
  function collectCtas() {
    return [...document.querySelectorAll(".cta-slot-card")].map(card => ({
      label: card.querySelector(".cta-label-input")?.value?.trim() || "",
      url: card.querySelector(".cta-url-input")?.value?.trim() || ""
    })).filter(v => v.label || v.url);
  }
  function collectMarquees() {
    return ["marquee_1","marquee_2","marquee_3"].map(id => document.getElementById(id)?.value?.trim() || "").filter(Boolean);
  }
  function buildNote() {
    const lines = [];
    ...
    if (ctas.length) {
      lines.push("CTA：");
      ctas.forEach((item, idx) => lines.push(`- CTA ${idx + 1}｜${item.label}｜${item.url}`));
    }
    const marquees = collectMarquees();
    if (marquees.length) {
      lines.push("跑馬燈：");
      marquees.forEach((m, idx) => lines.push(`- 第 ${idx + 1} 則：${m}`));
    }
    return lines.join("\\n");
  }
  async function submitForm(e) {
    ...
    try {
      ...
      const res = await fetch(CONFIG.GAS_URL, { method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify(payload) });
      const raw = await res.text();
      let data = {};
      try { data = JSON.parse(raw); } catch { throw new Error("GAS 回傳不是有效 JSON"); }
  ...
  function restoreDraft() {
    try {
      ...
      Object.entries(draft).forEach(([k,v]) => {
        ...
        if (el && k !== "plan" && k !== "addons") el.value = v;
      });
    } catch {}
  }
