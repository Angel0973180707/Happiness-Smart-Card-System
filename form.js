...
(() => {
  ...
  function init() {
    ...
    restoreDraft();
    syncAll();
    setStatus("目前版本：v6.8.5 真 UI 商業版");
  }
  ...
  function renderPhotos() {
    const prevFiles = [...els.photo_slots.querySelectorAll(".photo-card")].map(card => ({
      title: card.querySelector(".photo-title")?.textContent || ""
    }));
    ...
    for (let i = 1; i <= state.photoLimit; i++) {
      ...
      let rotation = 0, scale = 1, objectUrl = "";
