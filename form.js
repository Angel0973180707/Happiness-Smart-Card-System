/* ============================================================
   天使幸福智慧名片館 form.js
   v7.8.5-premium-banner-center-slogan
   - 精品款新增可選 premium_banner 上傳欄位
   - premium_banner_url 送入 preview 與 payload
   - 未上傳自動隱藏
   - 保留 waitAllUploads pending/uploading 等待修正
============================================================ */
...
(() => {
  ...
  const state = {
    photoMeta: {
      ...
      premium_banner: { ...DEFAULT_PHOTO_META }
    },
    photoPreviewUrls: {},
    photoRealUrls: {},
    photoFiles: {},
  ...
  function upgradeExperienceToTextarea() {
    ...
    if (!parent.querySelector(".experience-hint")) {
  ...
  function collectEls() {
    [
      ...
      "progress-contact-service", "invite_code", "ref", "addon_marquee_enabled", "addon_photo_enabled",
      "addon_photo_qty", "addon_cta_enabled", "addon_cta_qty", "addon_update_unlimited_enabled",
      "addon_bundle_enabled", "addon_agent_upgrade_enabled", "addon-photo-tip", "free-theme-group",
      "premium-theme-group", "free_color", "free_style", "free_paper", "premium_color", "display_name",
      "unit", "title", "phone", "email", "website", "line_url", "line_oa", "wechat_id", "experience",
      "services", "address", "intro", "video1", "video2", "video3", "social1", "social2", "social3",
      "marquee-section", "marquee_text", "photo-slots", "cta-slots", "livePreviewCard", "preview-theme-scope",
      ...
      "progress-success-panel", "progress-card-id-display", "btn-copy-card-notice", "progress-preview-link"
  ...
  function bindStaticEvents() {
    ...
    [
      ...
      "marquee_text", "video1", "video2", "video3", "social1", "social2", "social3"
    ].forEach(id => {
      if (!els[id]) return;
  ...
  function validateBeforeSubmit() {
    ...
    ["website", "line_url", "video1", "video2", "video3", "social1", "social2", "social3"].forEach(id => {
      const v = valueOf(id);
      if (v && !isLooksLikeUrl(v)) throw new Error(`「${id}」的網址格式看起來有誤，請確認是否以 https:// 開頭。`);
    });
  ...
  function pickPaymentDueAt(data) {
    return data?.payment_due_at || data?.card?.payment_due_at || data?.data?.payment_due_at || data?.quote?.payment_due_at || (() => {
  ...
  function buildLastSubmitResult({ cardId, previewUrl, paymentDueAt, totalAmount, planLabel, customerName }) {
    return { cardId, previewUrl, paymentDueAt, dueDateStr: formatDateYMD(paymentDueAt), totalAmount, planLabel, customerName };
  }
  ...
  function getAddonQty(id) { const n = Number(els[id]?.value || 0); return Number.isFinite(n) && n > 0 ? Math.floor(n) : 0; }
  ...
  function renderPhotoSections(wallLimit) {
    ...
    ensurePhotoMetaKey("premium_banner");
    ...
    if (plan === "premium") {
      sections.push({
        title: "精品 Banner",
        desc: "精品款可選上傳 1 張 Banner 圖；未上傳則成品自動隱藏。",
        keys: ["premium_banner"]
      });
    }
    ...
    sections.push({
      title: "照片牆",
      desc: `本次可上傳 ${wallLimit} 張（不含個人照、Logo${plan === "premium" ? "與 Banner" : ""}）`,
  ...
  function buildPhotoCard(key, sectionTitle, index) {
    ...
    const nameMap = { avatar: "個人照", logo: "Logo", premium_banner: "精品 Banner" };
    card.dataset.photoKey = key;
    title.textContent = nameMap[key] || `照片牆 ${index}`;
  ...
  const THEME_CLASSES = [
    ...
    "p1","p2","p3","p4","p5","p6","p7", "style-arch","style-flat","style-spot", "paper-1","paper-2","paper-3"
  ];
  ...
  function updatePreview() {
    ...
    try {
      renderer.renderCard(buildPreviewData(), {
        mode: "form", root, useExistingDom: false, qrMode: "preview", allowActions: false,
  ...
  function buildPreviewData() {
    ...
    const premiumBannerUrl = state.photoRealUrls.premium_banner || state.photoPreviewUrls.premium_banner || "";
    if (premiumBannerUrl) data.premium_banner_url = premiumBannerUrl;
  ...
  function buildPhotoMetaMap() {
    const out = {
      ...
      premium_banner: normalizePhotoMeta(state.photoMeta.premium_banner)
    };
  ...
  function buildPayload() {
    ...
    const payload = {
      ...
      invite_code: valueOf("invite_code"), referrer: valueOf("ref"),
      ...
      ...(state.photoRealUrls.premium_banner ? { premium_banner_url: state.photoRealUrls.premium_banner } : {}),
  ...
  async function submit(e) {
    ...
    try {
      ...
      setStatus(err.message || "資料驗證失敗", "warn");
      return;
    }
    ...
    try {
      ...
      if (!data || !data.ok) throw new Error(data?.error || data?.message || data?.raw || "建立名片失敗，請稍後再試。");

      setProgressStep(3, "正在整理報價資料…");
      const cardId = data.card_id || data.id || data.card?.id || data.card?.card_id || data.data?.card_id || data.data?.id || "";
      const previewUrl = cardId ? `${CONFIG.SHOWCASE_URL}index.html?id=${encodeURIComponent(cardId)}&view=1` : CONFIG.SHOWCASE_URL;
      const paymentDueAt = pickPaymentDueAt(data);
      setProgressStep(4, "正在寫入報價與預覽資料…");
      const result = buildLastSubmitResult({ cardId, previewUrl, paymentDueAt, totalAmount, planLabel: limits.planLabel, customerName: finalPayload.name });
      state.lastSubmitResult = result;

      localStorage.setItem(CONFIG.QUOTE_STORAGE_KEY, JSON.stringify({
        card_id: cardId, customer_name: finalPayload.name || "", plan_name: limits.planLabel || "方案",
        submitted_at: new Date().toISOString(), payment_notice: "請於 3 天內完成付款", payment_due_at: paymentDueAt,
        preview_url: previewUrl, plan_amount: Number(limits.planPrice || 0), addon_items: addonItems,
        addon_amount: Number(addonAmount || 0), total_amount: Number(totalAmount || 0)
      }));
  ...
  async function parseJsonSafe(res) {
    let raw = "";
    try { raw = await res.text(); } catch (e) { return { ok: false, error: "無法讀取回應內容", raw: "" }; }
  ...
  function valueOf(id) { return (document.getElementById(id) || els[id])?.value?.trim?.() || ""; }
