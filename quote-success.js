...
(() => {
  ...
  function formatAddonItems(items) {
    ...
    return items.map(item => item.qty && item.unit_price
      ? `${item.name}｜${item.qty} × ${money(item.unit_price)}｜${money(item.amount)}`
  ...
  function formatPaymentNotice(notice) {
    ...
    try { return JSON.stringify(notice, null, 2); }
    catch { return "請於 3 天內完成付款"; }
  }
