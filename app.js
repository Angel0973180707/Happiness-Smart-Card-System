...
function log_() { if (CONFIG.DEBUG) console.log("[HSC-v399.11]", ...arguments); }
function warn_() { if (CONFIG.DEBUG) console.warn("[HSC-v399.11]", ...arguments); }
...
function applyV382_() {
  ...
  // mode changes may affect logo position in CSS; we just refresh a tiny safety
 ...
 * Happiness Smart Card System — app.js (v399.11 COMPLETE OVERWRITE) 2/3
 * - Images: avatar/logo/photo wall (fallback candidates)
 * - Contact Dock: LINE官網 + LINE + 微信(複製ID) + 影音/社群(1~3) + Web
 * - Buttons: dynamic average (inline flex)
 * ================================ */
...
function collectPhotoUrls_(payloadNorm) {
  ...
  // ✅ 優先吃「照片_fast / 照片 / photos / photos_full」
...
/* ---------------------------
Dock buttons
- 動態平均：每顆 btn flex: 1 1 0
- 影音/社群：讀「影音平台1~3」「社群平台1~3」
--------------------------- */
...
function addDockBtn_(wrap, label, hrefOrFn, iconClass) {
  ...
  btn.style.minWidth = "118px"; // 手機兩欄更穩
...
function renderContactDock_(payloadNorm) {
  ...
  if (lo && /^https?:\/\//i.test(lo)) addDockBtn_(wrap, "LINE官網", lo, iconFor_("line"));
  ...
  // 4) 影音平台
  const vids = [text(video1), text(video2), text(video3)].filter(Boolean);
  vids.forEach((v, idx) => {
    const href = isUrl_(v) ? ensureHttp_(v) : `https://www.google.com/search?q=${encodeURIComponent(v)}`;
    addDockBtn_(wrap, `影音${idx + 1}`, href, iconFor_("video"));
  });

  // 5) 社群平台
  const socs = [text(soc1), text(soc2), text(soc3)].filter(Boolean);
  socs.forEach((v, idx) => {
    const href = isUrl_(v) ? ensureHttp_(v) : `https://www.google.com/search?q=${encodeURIComponent(v)}`;
    addDockBtn_(wrap, `社群${idx + 1}`, href, iconFor_("social"));
  });
  ...
  const a = text(addr);
  if (a) addDockBtn_(wrap, "導航", `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(a)}`, iconFor_("map"));

  dock.style.display = wrap.children.length ? "" : "none";
}/* ================================
 * Happiness Smart Card System — app.js (v399.11 COMPLETE OVERWRITE) 3/3
 * - Apply data to card (name/unit/title/slogan/service/exp)
