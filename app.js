...
function getIdFromUrl_(){ try{ const sp = getSearchParams_(); return sp.get("id") || ""; }catch{ return ""; } }
...
function buildCardApiUrl_(id){
  ...
  u.searchParams.set("action", "card"); u.searchParams.set("id", cid); u.searchParams.set("tenant", CONFIG.DEFAULT_TENANT);
  u.searchParams.set("ts", String(Date.now())); u.searchParams.set("v", CONFIG.VERSION);
  return u.toString();
}

function buildNormalizedPayload_(obj){
  if(!obj || typeof obj !== "object") return obj;
  const out = { __raw: obj }, lower = Object.create(null);
...
function setImgWithFallback_(imgEl, candidates){
  ...
  const tryNext = ()=>{ idx++; if(idx >= list.length) return; imgEl.src = list[idx] + (list[idx].includes("?") ? "&" : "?") + "t=" + Date.now() + "&v=" + encodeURIComponent(CONFIG.VERSION); };
...
function openUrl_(url){ const u = normalizeUrl_(url); if(!u) return; window.open(u, "_blank"); }
function openMapByAddress_(addr){ const a = text(addr); if(!a) return; const q = encodeURIComponent(a); window.open(`https://www.google.com/maps/search/?api=1&query=${q}`, "_blank"); }
...
const UI_STATE = { plan: "free", theme: "color-1", premiumTheme: "p1", style: "arch", paper: "paper-1" };
...
function setActiveAmong_(elements, activeEl){ (elements || []).forEach(el => { if(el === activeEl) el.classList.add("active"); else el.classList.remove("active"); }); }

function syncPlanUI_(){
  const freeBtn = qs("btnPlanFree"), premBtn = qs("btnPlanPremium"), freeControls = qs("free-controls"), premiumControls = qs("premium-controls"), badge = qs("premiumBadge");
  setBodyClassOneOf_(BODY_MODE_CLASSES, UI_STATE.plan === "premium" ? "mode-premium" : "mode-free");
  if(UI_STATE.plan === "premium"){
    if(freeControls) freeControls.style.display = "none"; if(premiumControls) premiumControls.style.display = ""; if(badge) badge.style.display = "";
    if(freeBtn) freeBtn.classList.remove("active"); if(premBtn) premBtn.classList.add("active");
  }else{
    if(premiumControls) premiumControls.style.display = "none"; if(freeControls) freeControls.style.display = ""; if(badge) badge.style.display = "none";
    if(premBtn) premBtn.classList.remove("active"); if(freeBtn) freeBtn.classList.add("active");
  }
  syncThemeUI_(); syncStyleUI_(); syncPaperUI_();
}

function syncThemeUI_(){
  const freeDots = qsa("#freeDotsRow .dot"), premiumDots = qsa("#premiumDotsRow .p-dot");
  removeBodyClasses_(BODY_FREE_THEME_CLASSES); removeBodyClasses_(BODY_PREMIUM_THEME_CLASSES);
  if(UI_STATE.plan === "premium"){
    document.body.classList.add(UI_STATE.premiumTheme || "p1");
    premiumDots.forEach(btn => { const expected = btn.getAttribute("aria-label"); if(expected === UI_STATE.premiumTheme){ btn.classList.add("active"); }else{ btn.classList.remove("active"); } });
    ...
    freeDots.forEach(btn => {
      ...
      if(match && map[match] === UI_STATE.theme){ btn.classList.add("active"); }else{ btn.classList.remove("active"); }
    });
...
function syncStyleUI_(){
  removeBodyClasses_(BODY_STYLE_CLASSES); document.body.classList.add(`style-${UI_STATE.style || "arch"}`);
  const buttons = qsa("#styleRow .btn-neo");
  buttons.forEach(btn => { const t = text(btn.textContent); const map = {"正拱":"arch","平直":"flat","晨曦":"spot"}; const v = map[t] || ""; if(v === UI_STATE.style) btn.classList.add("active"); else btn.classList.remove("active"); });
}

function syncPaperUI_(){
  removeBodyClasses_(BODY_PAPER_CLASSES); document.body.classList.add(UI_STATE.paper || "paper-1");
  const buttons = qsa("#paperRow .btn-neo");
  buttons.forEach(btn => { const t = text(btn.textContent); const map = {"棉紙":"paper-1","象牙紙":"paper-2","霧灰":"paper-3"}; const v = map[t] || ""; if(v === UI_STATE.paper) btn.classList.add("active"); else btn.classList.remove("active"); });
}

window.setPlan = function(plan, el){ UI_STATE.plan = (plan === "premium") ? "premium" : "free"; syncPlanUI_(); if(el){ const planBtns = [qs("btnPlanFree"), qs("btnPlanPremium")].filter(Boolean); setActiveAmong_(planBtns, el); } };
window.setTheme = function(theme, el){ if(!theme) return; if(/^color-\d+$/.test(theme)){ UI_STATE.plan = "free"; UI_STATE.theme = theme; }else if(/^p\d+$/.test(theme)){ UI_STATE.plan = "premium"; UI_STATE.premiumTheme = theme; } syncPlanUI_(); if(el){ const groupSel = UI_STATE.plan === "premium" ? "#premiumDotsRow .p-dot" : "#freeDotsRow .dot"; setActiveAmong_(qsa(groupSel), el); } };
window.setStyle = function(style, el){ const v = text(style); if(!["arch","flat","spot"].includes(v)) return; UI_STATE.style = v; syncStyleUI_(); if(el) setActiveAmong_(qsa("#styleRow .btn-neo"), el); };
window.setPaper = function(paper, el){ const v = text(paper); if(!["paper-1","paper-2","paper-3"].includes(v)) return; UI_STATE.paper = v; syncPaperUI_(); if(el) setActiveAmong_(qsa("#paperRow .btn-neo"), el); };

function initSelectionState_(){
  const body = document.body; if(!body) return;
  UI_STATE.plan = body.classList.contains("mode-premium") ? "premium" : "free";
  const freeTheme = BODY_FREE_THEME_CLASSES.find(c => body.classList.contains(c)); if(freeTheme) UI_STATE.theme = freeTheme;
  const premiumTheme = BODY_PREMIUM_THEME_CLASSES.find(c => body.classList.contains(c)); if(premiumTheme) UI_STATE.premiumTheme = premiumTheme;
  const styleClass = BODY_STYLE_CLASSES.find(c => body.classList.contains(c)); if(styleClass) UI_STATE.style = styleClass.replace(/^style-/, "");
  const paperClass = BODY_PAPER_CLASSES.find(c => body.classList.contains(c)); if(paperClass) UI_STATE.paper = paperClass;
  syncPlanUI_();
}

function renderAvatar_(p){
  const avatarRaw = pick(p, ["avatar_img_fast","avatar_img","avatar_url","個人照_fast","個人照"]);
  const img = qs("u-img"); if(!img) return;
  const u = normalizeImageUrl_(avatarRaw); currentAvatarUrlCache = u || "";
...
function renderLogo_(p){
  ...
  const wrap = qs("logoWrap"), img = qs("u-logo");
  if(!wrap || !img) return;
  const u = normalizeImageUrl_(logoUrl);
  if(!u){ wrap.style.display = "none"; img.removeAttribute("src"); return; }
  wrap.style.display = "flex"; img.style.borderRadius = "18px"; img.style.objectFit = "cover"; setImgWithFallback_(img, buildImgCandidates_(u));
}

function renderBlocks_(p){
  const service = pick(p, ["services","服務項目","service"]), exp = pick(p, ["experience","經歷","exp"]), b1 = qs("block-service"), b2 = qs("block-exp");
  if(b1){
    if(text(service)){ b1.style.display = ""; b1.innerHTML = `<div class="block-title">服務項目</div><div class="block-body preline">${escapeHtml_(service)}</div>`; }
    else{ b1.style.display = "none"; b1.innerHTML = ""; }
  }
  if(b2){
    if(text(exp)){ b2.style.display = ""; b2.innerHTML = `<div class="block-title">經歷</div><div class="block-body preline">${escapeHtml_(exp)}</div>`; }
    else{ b2.style.display = "none"; b2.innerHTML = ""; }
  }
}

function renderContactDock_(p){
  const dock = qs("contactDock"), btns = qs("contactButtons");
  if(!dock || !btns) return; btns.innerHTML = "";
  const phone = pick(p, ["phone","電話"]), email = pick(p, ["email","Email"]), address = pick(p, ["address","地址"]), lineUrl = normalizeUrl_(pick(p, ["line_url","line_oa","LINE連結"])), wechatId = text(pick(p, ["wechat_id","微信ID","微信"]));
  const list = [];
  if(lineUrl) list.push({label:"私訊 LINE", icon:"fa-brands fa-line", cls:"dock-line", action: ()=> openUrl_(lineUrl)});
  if(wechatId) list.push({label:"微信ID", icon:"fa-brands fa-weixin", cls:"dock-web", action: async ()=>{ try{ if(navigator.clipboard?.writeText) await navigator.clipboard.writeText(wechatId); }catch{} alert("✅ 已複製微信ID"); }});
  ...
  dock.style.display = ""; applyWideRule_(btns);
}
...
function renderPrimaryLinkDock_(p){
  const dock = qs("primaryLinkDock"), btns = qs("primaryLinkButtons");
  if(!dock || !btns) return;
  btns.innerHTML = "";
...
function renderMediaDock_(p){
  const dock = qs("mediaDock"), btns = qs("mediaButtons");
  if(!dock || !btns) return;
  btns.innerHTML = "";
  const items = [];
  ["video1","video2","video3"].forEach((k, i)=>{ const u = normalizeUrl_(pick(p, [k, `影音連結${i+1}`])); if(u) items.push({ kind:"video", idx:i+1, url:u }); });
  ["social1","social2","social3"].forEach((k, i)=>{ const u = normalizeUrl_(pick(p, [k, `社群連結${i+1}`])); if(u) items.push({ kind:"social", idx:i+1, url:u }); });
  if(!items.length){ dock.style.display = "none"; return; }
  items.forEach(item=>{ const meta = inferLinkMeta_(item.url, item.kind, item.idx); btns.appendChild(buildDockBtn_({ label: meta.label, icon: meta.icon, extraClass: meta.cls, onClick: ()=> openUrl_(item.url) })); });
  dock.style.display = ""; applyWideRule_(btns);
}

function renderCtaDock_(p){
  const dock = qs("ctaDock"), btns = qs("ctaButtons");
  if(!dock || !btns) return;
  btns.innerHTML = "";
  const ctaText = text(pick(p, ["cta_text","CTA文字","ctaText"])), ctaLink = normalizeUrl_(pick(p, ["cta_link","CTA連結","ctaLink"]));
  if(!ctaText || !ctaLink){ dock.style.display = "none"; return; }
  btns.appendChild(buildDockBtn_({ label: ctaText, icon: "fa-solid fa-bolt", extraClass: "dock-web wide", onClick: ()=> openUrl_(ctaLink) }));
  dock.style.display = "";
}

function renderInstallDock_(){
  const dock = qs("installDock"), cleanMode = getSearchParams_().get("view") === "1" || getSearchParams_().get("clean") === "1";
  if(!dock) return;
  dock.style.display = cleanMode ? "" : "none";
}

function renderDocks_(p){ renderInstallDock_(); renderContactDock_(p); renderPrimaryLinkDock_(p); renderMediaDock_(p); renderCtaDock_(p); }

function collectPhotos_(p){
  const keys = ["photo1_img_fast","photo2_img_fast","photo3_img_fast","photo4_img_fast","photo5_img_fast","photo1_img","photo2_img","photo3_img","photo4_img","photo5_img","photo1_url","photo2_url","photo3_url","photo4_url","photo5_url"];
  const out = [];
  keys.forEach(k=>{ const u = normalizeImageUrl_(pick(p, [k])); if(u && !out.includes(u)) out.push(u); });
  return out;
}

function renderPhotoWall_(p){
  const wall = qs("photoWall"), grid = qs("photoGrid");
  if(!wall || !grid) return;
  grid.innerHTML = "";
  const photos = collectPhotos_(p);
  if(!photos.length){ wall.style.display = "none"; return; }
  photos.forEach(u=>{ const img = document.createElement("img"); img.className = "wall-img"; img.alt = "照片"; img.loading = "lazy"; img.decoding = "async"; img.style.width = "100%"; img.style.aspectRatio = "1 / 1"; img.style.objectFit = "cover"; setImgWithFallback_(img, buildImgCandidates_(u)); img.addEventListener("click", ()=> openUrl_(u)); grid.appendChild(img); });
  wall.style.display = "";
}
...
function applySmartBalanceToEl_(el){
  ...
  if(el.dataset.balanceType === "hero-copy"){
    ...
    if(parts.length >= 2){
      ...
      const rest = parts.slice(1).join("｜"), idx = rest.indexOf("打造");
      if(idx > -1){
  ...
  if(el.dataset.balanceType === "qr-title" || el.dataset.balanceType === "product-qr-title"){
    if(raw.includes("｜")){ const arr = raw.split("｜"); el.innerHTML = `<span class="balance-line">${escapeHtml_(arr[0])}｜</span><span class="balance-line">${escapeHtml_(arr.slice(1).join("｜"))}</span>`; return; }
  }
  if(el.dataset.balanceType === "qr-sub" || el.dataset.balanceType === "product-qr-sub"){
    if(raw.includes("・")){
      const parts = raw.split("・").map(x=>x.trim()).filter(Boolean);
      if(parts.length >= 2){
        const half = Math.ceil(parts.length / 2), l1 = parts.slice(0, half).join("・"), l2 = parts.slice(half).join("・");
...
window.addEventListener("resize", ()=>{ clearTimeout(__balanceTimer); __balanceTimer = setTimeout(applySmartBalanceAll_, 120); });

function buildCleanShareUrl_(){
  try{
    const u = new URL(location.href);
    if(u.searchParams.get("view") !== "1") u.searchParams.set("view","1");
    u.searchParams.delete("clean"); u.searchParams.delete("admin");
    return u.toString();
  }catch{
...
function buildHubShareUrl_(){
  try{
    const u = new URL(CONFIG.HUB_URL), id = normalizeId_(getIdFromUrl_());
...
function buildBottomQrUrl_(){
  try{
    const u = new URL(location.href), id = normalizeId_(getIdFromUrl_()) || CONFIG.DEFAULT_ID;
    u.searchParams.set("id", id); u.searchParams.set("view", "1"); u.searchParams.delete("clean"); u.searchParams.delete("admin");
    return u.toString();
  }catch{
...
function buildQuickChartQrUrl_(url, size){ const s = Number(size) || 220; return "https://quickchart.io/qr?margin=1&ecLevel=H&size=" + encodeURIComponent(s) + "&text=" + encodeURIComponent(url); }

function setCenterImg_(imgEl, centerImgUrl){
  if(!imgEl) return;
  const u = normalizeImageUrl_(centerImgUrl);
  if(!u){ imgEl.removeAttribute("src"); imgEl.style.display = "none"; return; }
  imgEl.style.display = "block"; setImgWithFallback_(imgEl, buildImgCandidates_(u));
}

function renderQr(options){
  const { container, url, size = 160, colorDark = "#433227", colorLight = "#ffffff", centerImgEl = null, centerImgUrl = "" } = options || {};
  if(!container || !url) return false;
  container.innerHTML = "";
  let rendered = false;
  if(window.QRCode){
    try{
      new QRCode(container, { text: String(url), width: Number(size), height: Number(size), colorDark, colorLight, correctLevel: QRCode.CorrectLevel.H });
      rendered = !!container.querySelector("canvas, img");
    }catch(err){ console.error("renderQr QRCode failed:", err); }
  }
  if(!rendered){
...
function tryRenderBottomQrOnce_(p){
  const sec = qs("bottomQrSection"), grid = qs("bottomQrGrid"), avatar = qs("bottomQrAvatar");
  if(!sec || !grid || !avatar) return false;
  const qrUrl = buildBottomQrUrl_();
  const avatarRaw = pick(p, ["avatar_img_fast","avatar_img","avatar_url","個人照_fast","個人照"]), avatarUrl = normalizeImageUrl_(avatarRaw);
  const ok = renderQr({ container: grid, url: qrUrl, size: 136, colorDark: "#433227", centerImgEl: avatar, centerImgUrl: avatarUrl });
  sec.style.display = "";
  return ok;
}
...
function renderBottomQr_(p){
  ...
  const tick = ()=>{ attempts++; const ok = tryRenderBottomQrOnce_(p); if(ok) return; if(attempts < maxAttempts) setTimeout(tick, 220); };
  tick();
...
function refreshAllQrSafe_(){
  if(currentRow){
    try{ renderBottomQr_(currentRow); }catch(e){}
    try{ renderFeatureQrFromCurrent_(); }catch(e){}
  }
}
...
function renderCard(row){
  ...
  currentRow = p;
  const nameEl = qs("u-name"), unitEl = qs("u-unit"), titleEl = qs("u-title"), sloganEl = qs("u-slogan");
...
function updateInstallUi_(){
  const btn = qs("btnInstallCard"), fab = qs("installFab"), dock = qs("installDock");
...
(async function boot_(){
  try{
    initSelectionState_(); applySmartBalanceAll_(); updateInstallUi_();
    const id = getIdFromUrl_() || CONFIG.DEFAULT_ID, url = buildCardApiUrl_(id), payload = await fetchJsonRobust_(url), row = payload?.item || payload?.data || payload;
    renderCard(row);
    setTimeout(()=>{ try{ refreshAllQrSafe_(); }catch(e){} }, 240);
    setTimeout(()=>{ try{ refreshAllQrSafe_(); }catch(e){} }, 900);
  }catch(err){
    console.error(err);
    const nameEl = qs("u-name"), unitEl = qs("u-unit"), titleEl = qs("u-title");
